...
import './ActivityNewsPage.scss'

const STORAGE_KEY = 'esn_activity_all'

export type ActivityCategory = 'general' | 'ptcg' | 'ultraman' | 'result'
export type LayoutType = 'layout-left' | 'layout-right' | 'layout-top'
...
function save(data: ActivityItem[]) {
  localStorage.setItem(STORAGE_KEY, JSON.stringify(data))
}

export function ActivityNewsPage() {
  const [items, setItems] = useState<ActivityItem[]>(load)
  ...
  const [form, setForm] = useState({
    ...
    layout: 'layout-left' as LayoutType,
    startAt: '',
    endAt: '',
  })
  ...
  // --- 渲染組件：編輯時的版面預覽 ---
  const LayoutPreview = ({ item }: { item: any }) => {
    const config = categoryConfig[item.category as ActivityCategory] || categoryConfig.general
  ...
  // --- 渲染組件：列表項目 ---
  const AdminListItem = ({ item }: { item: ActivityItem }) => {
          ...
          <div className="list-controls">
            <div className="filter-tabs">
              ...
              {Object.entries(categoryConfig).map(([k, v]) => (
                <button key={k} onClick={() => setFilter(k as any)} className={`tab-btn ${filter === k ? 'active' : ''}`}>{v.label}</button>
              ))}
            </div>
