...
export type CompetitionType = 'ptcg' | 'ultraman' | 'result'
...
function save(data: CompetitionItem[]) {
  localStorage.setItem(STORAGE_KEY, JSON.stringify(data))
}

export function CompetitionPage() {
  const [items, setItems] = useState<CompetitionItem[]>(load)
  const [filter, setFilter] = useState<CompetitionType | 'all'>('all')
  ...
  const [form, setForm] = useState<{
    ...
    content: string
    imageUrl: string
  }>({ type: 'ptcg', title: '', content: '', imageUrl: '' })
  ...
  const openCreate = (type?: CompetitionType) => {
    setForm({
      type: type ?? 'ptcg',
      title: '',
      content: '',
  ...
  const openEdit = (item: CompetitionItem) => {
    setForm({
      type: item.type,
      title: item.title,
  ...
  return (
    <div className="competition-page">
      ...
      <div className="filter-section">
        ...
        {(Object.keys(typeLabels) as CompetitionType[]).map((type) => (
          <button
            key={type}
            type="button"
          ...
          <div className="form-grid">
            <div className="form-group">
              ...
              <select
                value={form.type}
                onChange={(e) =>
                  setForm((f) => ({ ...f, type: e.target.value as CompetitionType }))
                }
                className="form-select"
              >
                {(Object.entries(typeLabels) as [CompetitionType, string][]).map(
                  ([value, label]) => (
